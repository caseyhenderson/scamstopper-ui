...
describe('Testing the landing page of the Scamstopper web application, to ensure it works as expected.', () => {
    ...
    it('navigates to the email checker page, submits a mock scam email and displays the prediction and confidence values correctly.', () => {
      ...
      cy.get('[data-test-id="email-analysis-form"]').click()
      ...
      cy.wait(1000)
      cy.get('[data-test-id="prediction-result"]').should('have.text', 'Prediction: Hold up! Scamstopper thinks this could be a scam. Return to the chat window for more advice. ')
      cy.get('[data-test-id="confidence-result"]').should('exist')
